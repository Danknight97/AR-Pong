...
public class ImageTools {
    ...
    public static byte[] encodeImage(byte[] pixels, int width, int height, int format, int stride, int quality) {
        ...
        if (format == CAMERA_IMAGE_FORMAT_NV21) {
            ...
            ByteArrayOutputStream encodedBuffer2 = new ByteArrayOutputStream();
            if (bmp.compress(CompressFormat.JPEG, quality, encodedBuffer2)) {
                return encodedBuffer2.toByteArray();
            }
            return null;
        }
    }
